...
export default function SubcontractorsPage() {
  ...
  const filtered = providers.filter(p => p.type === "subcontractor");

  return (
    <div className="p-4 md:p-6 space-y-4">
      <div><h1 className="page-header">{t ? "Subcontratadas" : "Subcontractors"}</h1><p className="page-subheader">{t ? "Empresas subcontratadas" : "Subcontracted companies"}</p></div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map(p => (
          <div key={p.id} className="bg-card rounded-lg border p-4 hover:shadow-sm transition-shadow">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-9 h-9 rounded-lg bg-warning/10 flex items-center justify-center"><Building2 className="w-4 h-4 text-warning" /></div>
